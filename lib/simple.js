    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendFile: {
         ...
         let pase = messageType.replace('application', 'document') || messageType
          return await conn.sendMessage(jid, {[`${pase}`]: data,mimetype: mime, contextInfo: settings.freply ? ctx : null, ...options}, {quoted})}
        },
        sendContact: {
            /**
             * Send Contact
...
END:VCARD
        ...
        reply: {
            ...
            value(jid, text = '', quoted, options) {
            	let q = jid.endsWith('.net') ? global.fpayment(quoted) : quoted
                return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', q, false, options) : conn.sendMessage(jid, { ...options, text }, { quoted: q, ...options })
            }
        },
        sendListM: {
        	/**
        ...
        sendHydrated: {
            ...
            async value(jid, text = '', footer = '', buffer, url, urlText, call, callText, buttons, quoted, options) {
                ...
                let message = {
                    ...
                    [buffer ? 'caption' : 'text']: text || '',
                    footer,
                    contextInfo: settings.freply ? ctx : null,
                    templateButtons,
                    ...(buffer ?
                        options.asLocation && /image/.test(type.mime) ? {
                            location: {
        ...
        updateProfilePicture: {
        	/**
        	 * Update Profile Picture By Fokus ID
			 * @param {String} Jid
			 * @param {Buffer|String} Buffer
			 */
        	async value(jid, buffer) {
        		let { img } = await generateProfilePicture(buffer);
        		await conn.query({
        			tag: 'iq',
        			attrs: {
        				to: jid,
...
/**
 * Generate Profile Picture By Fokus ID
 * @params {string}
 * @returns Buffer
 */
async function generateProfilePicture(path) {
	let buffer = Buffer.isBuffer(path) ? path : /^data:.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await fetch(path)).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0);
	let buff = await jimp.read(buffer);
	let min = buff.getWidth();
	let max = buff.getHeight();
	let crop = buff.crop(0, 0, min, max);
	return {
